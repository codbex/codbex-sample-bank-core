...
export class CustomerRepository extends Repository<CustomerEntity> {
    ...
    public override findById(id: string | number, options?: Options): CustomerEntity | undefined {
        ...
        if (entity) {
            entity.dateOfBirth = entity.dateOfBirth ? new Date(entity.dateOfBirth) : undefined;
    ...
    protected override async triggerEvent(data: EntityEvent<CustomerEntity>): Promise<void> {
        const triggerExtensions = await Extensions.loadExtensionModules('codbex-sample-bank-core-edm-customers-Customer', ['trigger']);
        ...
        Producer.topic('codbex-sample-bank-core-edm-customers-Customer').send(JSON.stringify(data));
    }
}
